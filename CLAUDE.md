...
- Feature branches feed the persistent `codex/staging` branch. Test at
  `https://fleet-ops-staging.pages.dev` before requesting release to `main`.
- Cloudflare project `fleet-ops-staging` deploys `codex/staging`; both its
  primary deployment and feature previews use staging Supabase
  `netxgjqeaakbkjqvtdhl`. Use only staging accounts and test data.
- Cloudflare project `fleet-ops` deploys `main` to the live site and uses
  production Supabase `hjebavtcdduortshufku`. Automatic previews are disabled.
  Old production preview URLs may still access live data: never test there.
- Verify database targets before testing, including local `.env.local` and
  any CLI project link. A Git branch alone does not isolate data.
- Show the release diff and obtain explicit approval before merging into
  `main` (which automatically deploys production). Run typecheck, lint,
  tests, build, and relevant staging checks first.
- Hosted SQL still requires showing the exact SQL and target project and
  waiting for approval, including on staging. Never copy staging data into
  production or run seed/reset commands against production.
- See `docs/deployment.md` and decision 0024 for configuration and verification.

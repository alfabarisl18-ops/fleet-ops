...
/**
 * The app's mark: a simple line-art van in a rounded blue badge, plus the
 * real product name (never the old "FO" monogram or invented wordmark
 * copy). Used identically across the sidebar, the top bar, and all three
 * pre-auth screens (RoleChooser, DesktopSignIn, MobilePinSignIn) so the
 * brand reads the same before and after sign-in.
 */
export function Logo({ size = 36, withWordmark = true, className = '' }: LogoProps) {
  return (
    <div className={`flex items-center gap-2.5 ${className}`}>
      ...
      {withWordmark && <span className="font-heading text-base font-semibold text-slate-900">Fleet Operations SL</span>}
    </div>
  )
}
